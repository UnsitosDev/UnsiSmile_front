...
import { AdminResponse, User } from 'src/app/shared/models/shared/admin/admin.model';
import { Subject, debounceTime, distinctUntilChanged } from 'rxjs';
import { AdminTableData } from 'src/app/shared/models/shared/admin/admin';
...
@Component({
  selector: 'app-table-medical-admin',
  standalone: true,
  imports: [FormsModule,
    ReactiveFormsModule,
    MatCheckboxModule,
    ...
    MatCardModule,
    LoadingComponent],
  templateUrl: './table-medical-admin.component.html',
  styleUrl: './table-medical-admin.component.scss'
})
export class TableMedicalAdminComponent {
adminList: AdminTableData[] = [];
  ...
  edit(objeto: any) {
    this.router.navigate(['/admin/medical-admin/updateMedicalAdmin', objeto['numero empleado']]);
  }
  ...
  getAdmins(page: number = 0, size: number = 10, keyword: string = '') {
    ...
    const url = `${UriConstants.GET_MEDICAL_ADMIN}?page=${page}&size=${size}&keyword=${encodedKeyword}&order=${this.sortField}&asc=${this.sortAsc}`;
  ...
  onStatusChange(event: { row: any, newStatus: string }) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        const url = `${UriConstants.PATCH_MEDICAL_ADMIN}/${employeeNumber}/toggle-status`;
