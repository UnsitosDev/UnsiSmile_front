...
export abstract class MedicalRecordBaseComponent implements OnInit, OnDestroy {
  ...
  protected loadMedicalRecord(): void {
    this.historyService
      ...
      .subscribe({
        next: (mappedData: dataTabs) => this.handleMedicalRecordResponse(mappedData),
        complete: () => {
  ...
  private handleMedicalRecordResponse(mappedData: dataTabs): void {
    this.mappedHistoryData = this.medicalRecordBaseService.processMedicalRecordData(mappedData, this.role);
    this.updateCurrentSection();
